...
def get_Course_EveryThing(request, enterNum):#給課程id, 進入課程, 爬公告
    ...
    userName = unit.cName#在模板輸出使用者的姓名
    s= requests.session()
    ...
    n_File_Floder_url = "./user_Check/"+unit.cID+"/"+soup.title.string.strip()[15:]
    print(n_File_Floder_url)
    if not os.path.isdir(n_File_Floder_url):
        os.mkdir(n_File_Floder_url)
    ...
    for i in soup.find_all("span", id=re.compile("_lbBeginDate")):#公告日期
        announcement_time.append(i.string)
    for i in soup.find_all("span", id=re.compile("_lbContent")):#公告內容
        announcement_text.append(i.get_text())#因為有些會有在span中會有一些很奇怪的東西不能用string
    ...
    soup.decompose = True
    soup.clear()
    return render(request, "courseInfo.html", locals())#會跑到course.html模板

    #一起弄在一個html上所以應該是要抓一個soup
    #bfs算法