...
def post(request):
    ...
    if request.method == "POST":#以下是爬蟲程式
        ...
        try:
            ...
            unit.save()
            File_Floder_url = "./user_Check/"+unit.cID
            print(File_Floder_url)
            if not os.path.isdir(File_Floder_url):
                os.mkdir(File_Floder_url)
            for i in course_Name:
                n_File_Floder_url ="./user_Check/" +unit.cID+ "/" + str(i)+"/"
...
def check_ANN(course_URL, uid,upass):#給課程id, 進入課程, 爬公告
    s= requests.session()
    ...
    for i in soup.find_all("span", id=re.compile("_lbCaption")):#公告標題
        announcement_title.append(i.string)
...
def get_Course_EveryThing(request, enterNum):#給課程id, 進入課程, 爬公告
    ...
    announcement_info =[]#合起來放在html上一個for迴圈出來
    for i in soup.find_all("span", id=re.compile("_lbCaption")):#公告標題
    ...
    announcement_info= zip(announcement_title,announcement_time,announcement_text)#這要丟到html裡面 順序為 標題 日期 公告
    #############################
    #####存儲公告文件只存一次#######
    #########登入跳出通知##########
    #############################
    ...
    for u in soup.find_all("tr", class_= re.compile("Tbl")):
        course_DOC_Name.append(u.find("td").get_text())
    ...
    print(len(course_DOC_Total))#目前文件數量
    soup.decompose = True
    soup.clear()
